...
- **画板尺寸自适应**：选中一个 Frame 后开启"跟随宽高"，条数按 `floor((width + gap) / (barWidth + gap))` 自动计算，最大高度直接取节点高度；画板被 resize 时面板会实时跟上
...
- **尺寸实时跟随**：选中节点的宽高变了，面板和预览会一起变——拖手柄 resize、改右侧 W/H、以及按 `F` 拖出新画板的整个过程都算。实现上除了 `selectionchange`，主进程还监听了当前页的 `nodechange`，外加 UI 侧 300ms 一次的兜底轮询；光靠 `selectionchange` 会卡在刚选中时的那个尺寸上。
- 取消选中时开关只是停用、不清零：跟随意图会保留，重新选中节点立刻恢复生效，也会跨会话记住。
- 生成位置：有选中时放进最近的可收纳容器（选中的画板本身，或选中图层所在的画板）并居中；选中项直接躺在页面根级、或什么都没选时，才落在视口中心。
