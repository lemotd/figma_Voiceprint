...
- **画板宽度自适应**：选中一个 Frame 后开启"跟随宽度"，条数按 `floor((width + gap) / (barWidth + gap))` 自动计算
...
选中任意有宽度的节点（Frame、Group、矩形…），面板顶部会显示节点名 / 类型 / 宽 × 高，同时启用「跟随宽度」开关。

- 勾选后，条数字段被锁定，值由 `barWidth` 和 `gap` 反推：`count = floor((width + gap) / (barWidth + gap))`，至少 4。
